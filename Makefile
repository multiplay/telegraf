...
GOFMT ?= $(shell gofmt -l -s $(filter-out plugins/parsers/influx/machine.go, $(GOFILES), $(wildcard vendor/*)))
BUILDFLAGS ?=
...
.PHONY: fmt
fmt:
	@gofmt -s -w $(filter-out plugins/parsers/influx/machine.go, $(GOFILES), $(wildcard vendor/*))
